...
/// Builds a `HashMap` from transformed items. Later items overwrite earlier
/// items when they produce the same key.
///
/// **Time Complexity:**  
...
/// # Arguments
///
/// * `collection` - Items to transform.
/// * `transform` - A function returning `(key, value)` for an item.
///
/// # Type Parameters
///
/// * `T` - Item type.
...
/// # Returns
///
/// * `HashMap<K, V>` - A map of the transformed pairs.
///
/// # Examples
///
/// ```rust
